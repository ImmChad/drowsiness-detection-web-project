...
@section('contentAdmin')
    ...
    <div class="section-update-video">
        <div class="card-title" style="display: flex;justify-content: space-between">
            ...
            <div class="col-sm-5" style="display: flex; justify-content: flex-end;">
                <button class="button-ads btn-all-videos" style="margin-right: 0.5rem;">
                    All Videos
                </button>
                    ...
                    <div class="row" style="height: 350px; width: 250px; justify-content: center;">
                        <div class="col-md-12" style="width: 100%; height: 100%; border: 5px solid #0e5904; border-radius: 20px; padding: 0px !important; overflow: hidden;">
                            <div class="scroll-div-all-group" style="display: flex; width: 100%; height: 100%; flex-direction: column; justify-content: flex-start; align-items: flex-start;   font-size: 20px; overflow-y: scroll;">
                                <span class="choose-group-company" style="width: 100%; font-weight: 700; color: #0e5904; cursor: pointer;" company_id="0" id="company_id0">&nbsp;All</span>
                                @foreach($dataCompany as $subListDataCompany)
                                    @if($subListDataCompany->parent_id == 0)
                                        <span class="choose-group-company" style="width: 100%; font-weight: 700; color: #0e5904; cursor: pointer;" id="company_id{{ $subListDataCompany->company_id }}" company_id="{{ $subListDataCompany->company_id }}">&nbsp;{{ $subListDataCompany->company_group }}</span>
                                    @else
                                        <span class="choose-group-company" style="width: 100%; font-weight: 450; color: #0e5904; cursor: pointer;" id="company_id{{ $subListDataCompany->company_id }}" company_id="{{ $subListDataCompany->company_id }}">&nbsp;&nbsp; {{ $subListDataCompany->company_group }}</span>
                                    @endif
                                @endforeach
                            </div>
                        </div>
    ...
    <script>
        ...
        $('.btn-all-videos').click(function() {
            window.location.href = "/video/list-company-video";
        });
        ...
        itemCompanies.forEach(itemCompany=>{
                itemCompany.addEventListener('click',(event)=>{
                    location.href = `/video/list-company-photo/${event.currentTarget.getAttribute('company_id')}#company_id${event.currentTarget.getAttribute('company_id')}`
                })
        })
